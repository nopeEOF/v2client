...
from v2client.utils import to_any_type
...
class ProxyAPI(V2RayBase):
    ...
    def add_user(self, inbound_tag: str, email: str, proxy_type: str, level: int, **kwargs):
        ...
        try:
            stub.AlterInbound(command_pb2.AlterInboundRequest(
                tag=inbound_tag,
                operation=to_any_type(command_pb2.AddUserOperation(
                    user=user_pb2.User(
                        email=email,
                        level=level,
                        account=to_any_type(account)
                    )
                ))
            ))
    ...
    def remove_user(self, inbound_tag: str, email: str):
        ...
        try:
            stub.AlterInbound(command_pb2.AlterInboundRequest(
                tag=inbound_tag,
                operation=to_any_type(command_pb2.RemoveUserOperation(
                    email=email
                ))
            ))
        except grpc.RpcError as e:
